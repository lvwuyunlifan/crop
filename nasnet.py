...
class NASNetALarge(nn.Module):
    ...
    def __init__(self, num_classes, stem_filters=96, penultimate_filters=4032, filters_multiplier=2):
        ...
        self.fc = nn.Sequential(
            ...
            nn.Dropout(0.8),
            nn.Linear(24*filters, num_classes),
        )
...
def nt(num_classes, pretrained='imagenet'):
...
#     proto=torch.onnx.export(model,input,"nasnet.proto",verbose=True) #将model的结构和参数全部保存为 resnet32.proto
#     writer=tensorboardX.SummaryWriter("./logs/")  #定义一个tensorboardX的写对象
#     writer.add_graph_onnx("./nasnet.proto")  #将proto格式的文件转换为tensorboard中的graph
#     output = model(input)
#     print(output.size())
