...
from models_old import RACNN123, pairwise_ranking_loss, multitask_loss
...
decay_steps = [10, 20, 15]  # based on epoch

net = RACNN123(num_classes=61)
...
logger = Logger('./visual123/' + 'RACNN_CUB200_9')
...
if args.cuda:
    cls_params = list(net.module.b1.parameters()) + list(net.module.b2.parameters()) + list(
        net.module.b3.parameters()) + list(net.module.classifier1_2.parameters()) + list(
        net.module.classifier2.parameters()) + list(net.module.classifier1_2_3.parameters())
...
# opt1 = optim.Adam(cls_params, lr=args.lr, betas=(0.9, 0.999), eps=1e-08, weight_decay=5e-5)
...
# opt2 = optim.Adam(apn_params, lr=args.lr, betas=(0.9, 0.999), eps=1e-08, weight_decay=5e-5)
...
# 创建保存模型和结果的文件夹
if not os.path.exists('./racnn123_models'):
    os.makedirs('./racnn123_models')
if not os.path.exists('./result_racnn123/result'):
    os.makedirs('./result_racnn123/result')
# 创建日志文件
if not os.path.exists('./result_racnn123/%s.txt' % file_name):
    with open('./result_racnn123/%s.txt' % file_name, 'w') as acc_file:
        pass
with open('./result_racnn123/%s.txt' % file_name, 'a') as acc_file:
...
def save_checkpoint(type, state):
    filename = './racnn123_models/checkpoint_%s.pth.tar' % type
    if type == 'pre_APN':
...
def train(trainset, trainloader, valloader):
    ...
    # pretrain APN
    if resume_apn:
        checkpoint_path = './racnn123_models/checkpoint_pre_APN.pth.tar'
    ...
    while ((old_cls_loss - new_cls_loss) ** 2 > 1e-7) and ((old_apn_loss - new_apn_loss) ** 2 > 1e-7) and (
        ...
        while ((cls_tol < 10) and (cls_iter % 2000 != 0)):
        ...
        switch_step += 1
        while ((apn_tol < 10) and apn_iter % 2000 != 0):
            if (not batch_iterator) or (iteration % epoch_size == 0):
                batch_iterator = iter(trainloader)
            ...
            if (apn_iter % 20) == 0:
                ...
                # 在日志文件中记录每个epoch的精度和loss
                with open('./result_racnn123/%s.txt' % 'APN', 'a') as acc_file:
        ...
        for i, acc in enumerate(accuracy_list):
            ...
            if acc > accuracy_max:
                number = i
                # 在日志文件中记录每个epoch的精度和loss
        with open('./result_racnn123/%s.txt' % 'accuracy', 'a') as acc_file:
            acc_file.write(" accuracy1 = %.3f || accuracy12 =  %.3f || accuracy123 = %.3f || accuracy_best_number = %.3f \n" %
...
def train_cls(trainloader, iteration):
    ...
    for index, (images, labels) in enumerate(trainloader):
        ...
        if (index % 10) == 0:
            print(" [*] cls_epoch[%d] || dataIter[%d] %d || cls_loss: %.4f || Timer: %.4fsec" % (
                iteration, all_iteration, index, new_cls_loss.item(), (t1 - t0)))
            # 在日志文件中记录每个epoch的精度和loss
            with open('./result_racnn123/%s.txt' % 'cls', 'a') as acc_file:
                acc_file.write(" [*] cls_epoch[%d] || dataIter[%d] %d || cls_loss: %.4f || Timer: %.4fsec" % (
                    iteration, all_iteration, index, new_cls_loss.item(), (t1 - t0)))
...
def pretrainAPN(trainset, trainloader):
    ...
    for _iter in range(0, 20000):
        ...
        if iteration % int(epoch_size/10) == 0:
            apn_epoch += 1
            if apn_epoch in decay_steps:
                apn_steps += 1
                adjust_learning_rate(opt2, 0.1, apn_steps, args.lr)
        ...
        if (iteration % 20) == 0:
            ...
            with open('./result_racnn123/%s.txt' % 'pre_apn', 'a') as acc_file:
...
def validate(valloader, iteration, type):
    with torch.no_grad():
        ...
        if type == 'cls':
            print(" cls: [*] Iter %d || Val accuracy1: %.2f || Val accuracy12: %.2f || Val accuracy123: %.2f || Timer: %.4fsec" %
                    (iteration, accuracy1.item(), accuracy2.item(), accuracy3.item(), (t1 - t0)))
            with open('./result_racnn123/%s.txt' % 'cls_val_accuracy', 'a') as acc_file:
                acc_file.write(" cls: [*] Iter %d || Val accuracy1: %.2f || Val accuracy12: %.2f || Val accuracy123: %.2f || Timer: %.4fsec\n" %
                            (iteration, accuracy1.item(), accuracy2.item(), accuracy3.item(), (t1 - t0)))
        else:
            print(" apn: [*] Iter %d || Val accuracy1: %.2f, Val accuracy21: %.2f, Val accuracy123: %.2f || Timer: %.4fsec" %
                    (iteration, accuracy1.item(), accuracy2.item(), accuracy3.item(), (t1 - t0)))
            with open('./result_racnn123/%s.txt' % 'apn_val_accuracy', 'a') as acc_file:
                acc_file.write(" cls: [*] Iter %d || Val accuracy1: %.2f || Val accuracy12: %.2f || Val accuracy123: %.2f || Timer: %.4fsec\n" %
                            (iteration, accuracy1.item(), accuracy2.item(), accuracy3.item(), (t1 - t0)))
    net.train()
...
def test(test_loader, net):
    ...
    with open('./result_racnn123/result/submit1.json', "w") as f:
    ...
    # print('saa', sub_list[0:10])
    with open('./result_racnn123/result/submit12.json', "w") as f:
    ...
    # print('saa', sub_list[0:10])
    with open('./result_racnn123/result/submit123.json', "w") as f:
...
if __name__ == '__main__':
    ...
    train_data_list = pd.read_csv('./AgriculturalDisease_trainingset/label.csv')
    #
    ...
    valloader = data.DataLoader(valset, batch_size=args.batch_size,
                                shuffle=True, collate_fn=CUB_collate, num_workers=args.num_workers)
    ...
    #     break
    print('-----------',len(trainset)//4)
    train(trainset, trainloader, valloader)

    # 读取最佳模型，预测测试集，并生成可直接提交的结果文件
    print('test--------------------------------')
    # best_model = torch.load('./racnn_models/checkpoint_%d.pth.tar' % 'cls_and_rank')
    # net.load_state_dict(best_model['state_dict'])
    test(test_loader=testloader, net=net)
    print(" [*] Train done")            
